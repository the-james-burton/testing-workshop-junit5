...
You should now review the ```BookingServiceImplTest``` class and implement the remaining methods. Note the ```@ExtendWith(MockitoExtension.class)``` class level annotation which is required to activate the Mockito annotations you see on the class-level variables. Use ```Mockito.when()``` and ```Mockito.verify()``` features to mock calls to the external NetworkService dependency and verify that the calls were made by the Booking service under test. Use **[Mockito ArgumentMatchers](https://www.baeldung.com/mockito-argument-matchers)** to help ensure your verifications are correct.
